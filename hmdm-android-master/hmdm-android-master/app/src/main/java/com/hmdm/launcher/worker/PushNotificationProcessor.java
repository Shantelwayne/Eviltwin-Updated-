...
public class PushNotificationProcessor {
    ...
    private static void purgeDir(Context context, JSONObject payload) {
        ...
        try {
            ...
            // listFiles() returns null if an I/O error occurs — guard against NPE
            if (childFiles == null) {
                RemoteLogger.log(context, Const.LOG_WARN, "Directory purge failed: could not list files in: " + path);
                return;
            }
            for (File child : childFiles) {
    ...
    /**
     * Validates a shell command against the allowlist and injection pattern blocklist.
     *
     * Returns null if the command is permitted.
     * Returns a rejection reason string if the command should be blocked.
     */
    private static String validateCommand(String command) {
        ...
        // 1. Check for shell injection characters — block these in all cases
        for (String pattern : Const.SHELL_INJECTION_PATTERNS) {
            if (command.contains(pattern)) {
                return "contains forbidden shell injection pattern: '" + pattern + "'";
            }
        }

        // 2. Command must start with an allowlisted prefix
        for (String allowed : Const.SHELL_COMMAND_ALLOWLIST) {
            if (trimmed.startsWith(allowed.toLowerCase(java.util.Locale.ROOT))) {
                return null; // permitted
            }
        }

        return "command prefix not in allowlist";
    }
    ...
    private static void runCommand(Context context, JSONObject payload) {
        ...
        try {
            ...
            String rejection = validateCommand(command);
            if (rejection != null) {
                RemoteLogger.log(context, Const.LOG_WARN,
                        "Command blocked by security policy (" + rejection + "): " + command);
                return;
            }
