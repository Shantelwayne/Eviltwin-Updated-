...
public class Const {
    ...
    /**
     * Allowlist of permitted shell command prefixes for remote execution via push messages.
     * Commands from the server must start with one of these prefixes (case-insensitive trimmed).
     * Any command that doesn't match is rejected and logged.
     *
     * Permitted categories:
     *   am      — Activity Manager: start/stop activities, services, broadcasts
     *   pm      — Package Manager: grant/revoke permissions, clear data
     *   settings — Read/write Android system settings
     *   setprop  — Set system properties (requires root/system)
     *   input    — Inject key/touch events (used by aPuppet remote control)
     *   svc      — System service control (wifi, bluetooth, data)
     *   chmod    — Change file permissions (used for Datawedge config files)
     *   wm       — Window Manager: set display size/density
     *   dumpsys  — Dump system service state (diagnostic read-only)
     *   logcat   — Read system logs (diagnostic read-only)
     */
    public static final String[] SHELL_COMMAND_ALLOWLIST = {
        "am ",
        "pm ",
        "settings ",
        "setprop ",
        ...
        "dumpsys ",
        "logcat",
        "am\t", "pm\t", "settings\t"   // tab-separated variants
    };

    /**
     * Shell injection patterns that are always blocked, even for allowlisted commands.
     * These characters can chain or redirect commands regardless of the prefix.
     */
    public static final String[] SHELL_INJECTION_PATTERNS = {
        ";", "&&", "||", "|", "`", "$(", "${", ">", "<", "\n", "\r"
    };
}
