...
public class SettingsHelper {
    ...
    public static SettingsHelper getInstance(Context context) {
        if (instance == null) {
            // Always use applicationContext so the singleton never holds a reference
            // to a short-lived Context (e.g. Activity), which would cause a memory leak.
    ...
    public SettingsHelper(Context context) {
        // context is already applicationContext when called from getInstance()
    ...
    public void refreshConfig(Context context) {
        if (config == null) {
            sharedPreferences = context.getApplicationContext()
                    .getSharedPreferences(PACKAGE_NAME + PREFERENCES_ID, Context.MODE_PRIVATE);
            initConfig();
        }
    }
